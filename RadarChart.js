 ...
 */
InitRadarChart = function (domConstruct) {
	debugger;
	...
	if (!this.constructor.prototype._proto) {
		this.constructor.prototype._proto = {
			...
			_getChartOptions: function(view){
				...
				var chartOptions = {
					...
					onClick: function(e){
						var radarItem = this.getElementAtEvent(e) && this.getElementAtEvent(e)[0] ? this.getElementAtEvent(e)[0] : null;
						if(radarItem){
							var data = radarItem._chart.data;
							var datasetIndex = radarItem._datasetIndex;
							radarItem.label = data.datasets[datasetIndex].label;
							radarItem.value = data.datasets[datasetIndex].data[radarItem._index];
							bpmext.ui.executeEventHandlingFunction(view, "eventON_CLICK", {radarItem: radarItem});
						}
					},
				}
			...
			_mapData: function(view, radarChartData){
				...
				if(radarChartData){
					if(radarChartData && radarChartData.datasets && radarChartData.datasets.items && radarChartData.datasets.items.length > 0){
						for(var i=0; i<radarChartData.datasets.items.length; i++){
							var randomColor = colors[Common.utils.getRandomInt(0, colors.length-1)];
							var dataObj = {};
							var item = radarChartData.datasets.items[i];
							...
							dataObj.lineTension = item.lineTension ? item.lineTension : 0.000001;
					...
					labels = radarChartData && radarChartData.labels && radarChartData.labels.items ? radarChartData.labels.items : [];
				}

				return {labels: labels, datasets: datasets};
				
			},
			_getRadarChartData: function(view){
				return this._mapData(view, view.context.binding.get("value"));
			},
			_getRadarChartDataFromService: function(view){
				var dataServiceInput = Common.utils.getOptionValue(view, "dataServiceInput", null);
				var radarChartOptions = this._getChartOptions(view);
				if(view.myRadar){
					view.myRadar.destroy();
				}
				var ctx = view.canvas.getContext('2d');
				view.myRadar = new Chart(ctx, {
					type: "radar",
				...
				var serviceArgs = {
					...
					load : function (response) {
						if(response && response.radarChartData){
							view.context.binding.get("value").set("labels", response.radarChartData.labels);
							view.context.binding.get("value").set("datasets", response.radarChartData.datasets);
						}
					},
			...
			_createChart : function(view){
				...
				var ctx = view.canvas.getContext('2d');
				var radarChartData = this._getRadarChartData(view);
				var radarChartOptions = this._getChartOptions(view);
				if(radarChartData && radarChartData != undefined && radarChartOptions && radarChartOptions != undefined){
					view.myRadar = new Chart(ctx, {
						type: "radar",
						data: radarChartData,
						options: radarChartOptions
		...
		this.constructor.prototype.load = function () {
			...
			if(dataSource == "service"){
				this._proto._getRadarChartDataFromService(this);
			}else{
		...
		this.constructor.prototype.change = function (event) {
			...
			if(event.property == "datasets" || event.property == "labels"){
				...
				this.myRadar.config.data.datasets.forEach(function(dataset) {
				...
				var radarChartOptions = this._proto._getChartOptions(this);
				this.myRadar.options = radarChartOptions;
				this.myRadar.update();
