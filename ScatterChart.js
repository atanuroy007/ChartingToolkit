 ...
 */
InitScatterChart = function (domConstruct) {
	debugger;
	...
	if (!this.constructor.prototype._proto) {
		this.constructor.prototype._proto = {
				...
				var chartOptions = {
					...
					plugins: {
						datalabels: {
							...
							borderRadius: Common.utils.getValue(onChartLabelProperties, "borderRadius", null),
							borderWidth: Common.utils.getValue(onChartLabelProperties, "borderWidth", null),
							...
							formatter: function(value, context) {
								...
								return context.customValue ? context.customValue : "(X: "+value.x+", Y: "+value.y+")";
							},
					...
					onClick: function(e){
						var scatterItem = this.getElementAtEvent(e) && this.getElementAtEvent(e)[0] ? this.getElementAtEvent(e)[0] : null;
						if(scatterItem){
							var data = scatterItem._chart.data;
							var datasetIndex = scatterItem._datasetIndex;
							scatterItem.label = data.datasets[datasetIndex].label;
							scatterItem.value = data.datasets[datasetIndex].data[scatterItem._index];
							bpmext.ui.executeEventHandlingFunction(view, "eventON_CLICK", {scatterItem: scatterItem});
						}
					},
				};
			...
			_mapData: function(view, scatterChartData){
				...
				if(scatterChartData){
					if(scatterChartData && scatterChartData.items && scatterChartData.items.length > 0){
						for(var i=0; i<scatterChartData.items.length; i++){
							var randomColor = colors[Common.utils.getRandomInt(0, colors.length-1)];
							var dataObj = {};
							var item = scatterChartData.items[i];
							...
							dataObj.showLine = (item.showLine == "true") ? true : false;
							dataObj.spanGaps = (item.spanGaps == "true") ? true : false;
					...
					labels = scatterChartData && scatterChartData.labels && scatterChartData.labels.items ? scatterChartData.labels.items : [];
					xLabels = scatterChartData && scatterChartData.xLabels && scatterChartData.xLabels.items ? scatterChartData.xLabels.items : [];
					yLabels = scatterChartData && scatterChartData.yLabels && scatterChartData.yLabels.items ? scatterChartData.yLabels.items : [];
				}

				return {labels: labels, datasets: datasets};
				
			},
			_getScatterChartData: function(view){
				return this._mapData(view, view.context.binding.get("value"));
			},
			_getScatterChartDataFromService: function(view){
				var dataServiceInput = Common.utils.getOptionValue(view, "dataServiceInput", null);
				var scatterChartOptions = this._getChartOptions(view);
				if(view.myScatter){
					view.myScatter.destroy();
				}
				var ctx = view.canvas.getContext('2d');
				view.myScatter = new Chart(ctx, {
					type: "scatter",
				...
				var serviceArgs = {
					...
					load : function (response) {
						if(response && response.scatterChartData){
							view.context.binding.set("value", response.scatterChartData);
						}
					},
			...
			_createChart : function(view){
				...
				var ctx = view.canvas.getContext('2d');
				var scatterChartData = this._getScatterChartData(view);
				var scatterChartOptions = this._getChartOptions(view);
				if(scatterChartData && scatterChartData != undefined && scatterChartOptions && scatterChartOptions != undefined){
					view.myScatter = new Chart(ctx, {
						type: "scatter",
		...
		this.constructor.prototype.load = function () {
			...
			if(dataSource == "service"){
				this._proto._getScatterChartDataFromService(this);
			}else{
		...
		this.constructor.prototype.change = function (event) {
			debugger;
			if(event.property == "scatterChartData" && (event.oldVal != event.newVal)){
				this._proto._createChart(this);
			}else if(event.property == "colorScheme" && (event.oldVal != event.newVal)){
				this.colorScheme = event.newVal;
				this.myScatter.config.data.datasets.forEach(function(dataset) {
				...
				var scatterChartOptions = this._proto._getChartOptions(this);
				this.myScatter.options = scatterChartOptions;
