 ...
 */
InitLineChart = function (domConstruct) {
	debugger;
	...
	if (!this.constructor.prototype._proto) {
		this.constructor.prototype._proto = {
			...
			_getChartOptions: function(view){
				...
				var chartOptions = {
					...
					plugins: {
						datalabels: {
							...
							backgroundColor: function(context) {
								return (context.dataset.backgroundColor ? context.dataset.backgroundColor : context.dataset.tempBackgroundColor);
							},
					...
					onClick: function(e){
						var lineItem = this.getElementAtEvent(e) && this.getElementAtEvent(e)[0] ? this.getElementAtEvent(e)[0] : null;
						if(lineItem){
							var data = lineItem._chart.data;
							var datasetIndex = lineItem._datasetIndex;
							lineItem.label = data.datasets[datasetIndex].label;
							lineItem.value = data.datasets[datasetIndex].data[lineItem._index];
							bpmext.ui.executeEventHandlingFunction(view, "eventON_CLICK", {lineItem: lineItem});
						}
					},
				};
			...
			_mapData: function(view, lineChartData){
				...
				if(lineChartData){
					if(lineChartData && lineChartData.datasets && lineChartData.datasets.items && lineChartData.datasets.items.length > 0){
						for(var i=0; i<lineChartData.datasets.items.length; i++){
							var randomColor = colors[Common.utils.getRandomInt(0, colors.length-1)];
							var dataObj = {};
							var item = lineChartData.datasets.items[i];
							if(view.colorScheme == "" || !view.colorScheme || view.colorScheme == undefined){
								...
								dataObj.pointBackgroundColor = item.pointBackgroundColor ? item.pointBackgroundColor : color(window.chartColors[randomColor]).alpha(0.5).rgbString();
								dataObj.pointBorderColor = item.pointBorderColor ? item.pointBorderColor : window.chartColors[randomColor];
								dataObj.pointHoverBackgroundColor = item.pointHoverBackgroundColor ? item.pointHoverBackgroundColor : color(window.chartColors[randomColor]).alpha(0.5).rgbString();
								dataObj.pointHoverBorderColor = item.pointHoverBorderColor ? item.pointHoverBorderColor : window.chartColors[randomColor];
							}
							dataObj.borderCapStyle = item.borderCapStyle ? item.borderCapStyle  : 'butt';
							dataObj.borderDash = item.borderDash && item.borderDash.items ? item.borderDash.items : [];
							dataObj.borderDashOffset = item.borderDashOffset ? item.borderDashOffset : 0.0;
							dataObj.borderJoinStyle = item.borderJoinStyle ? item.borderJoinStyle : 'miter';
							dataObj.borderWidth = item.borderWidth ? item.borderWidth : 3;
							dataObj.cubicInterpolationMode = item.cubicInterpolationMode ? item.cubicInterpolationMode : "";
							dataObj.fill = (item.fill == "true") ? true : ((item.fill == "false" || item.fill == "" || item.fill == undefined || !item.fill) ? false : item.fill);
							dataObj.label = item.label ? item.label : "";
							dataObj.lineTension = item.lineTension ? item.lineTension : 0.4;
							dataObj.pointBorderWidth = item.pointBorderWidth ? item.pointBorderWidth : 2;
							dataObj.pointHitRadius = item.pointHitRadius ? item.pointHitRadius : 1;
							dataObj.pointHoverBorderWidth = item.pointHoverBorderWidth ? item.pointHoverBorderWidth : 3;
							dataObj.pointHoverRadius = item.pointHoverRadius ? item.pointHoverRadius : 4;
							dataObj.pointRadius = item.pointRadius ? item.pointRadius : 3;
							dataObj.pointRotation = item.pointRotation ? item.pointRotation : 0;
							dataObj.pointStyle = item.pointStyle ? item.pointStyle : 'circle';
							dataObj.showLine = (item.showLine == "false") ? false : true;
							dataObj.spanGaps = (item.spanGaps == "false") ? false : true;
							dataObj.steppedLine = (item.steppedLine == "true") ? true : ((item.steppedLine == "false" || item.steppedLine == "" || item.steppedLine == undefined || !item.steppedLine) ? false : item.steppedLine);
					...
					labels = lineChartData && lineChartData.labels && lineChartData.labels.items ? lineChartData.labels.items : [];
					xLabels = lineChartData && lineChartData.xLabels && lineChartData.xLabels.items ? lineChartData.xLabels.items : [];
					yLabels = lineChartData && lineChartData.yLabels && lineChartData.yLabels.items ? lineChartData.yLabels.items : [];
				}

				return {labels: labels, datasets: datasets};
				
			},
			_getLineChartData: function(view){
				return this._mapData(view, view.context.binding.get("value"));
			},
			_getLineChartDataFromService: function(view){
				var dataServiceInput = Common.utils.getOptionValue(view, "dataServiceInput", null);
				var linerChartOptions = this._getChartOptions(view);
				if(view.myLine){
					view.myLine.destroy();
				}
				var ctx = view.canvas.getContext('2d');
				view.myLine = new Chart(ctx, {
					type: "line",
					data: {labels: [], datasets: []},
					options: linerChartOptions
				});
				var serviceArgs = {
					params: {
					...
					load : function (response) {
						if(response && response.lineChartData){
							view.context.binding.get("value").set("labels", response.lineChartData.labels);
							view.context.binding.get("value").set("datasets", response.lineChartData.datasets);
						}
					},
			...
			_createChart : function(view){
				...
				var ctx = view.canvas.getContext('2d');
				var lineChartData = this._getLineChartData(view);
				var lineChartOptions = this._getChartOptions(view);
				if(lineChartData && lineChartData != undefined && lineChartOptions && lineChartOptions != undefined){
					view.myLine = new Chart(ctx, {
						type: "line",
						data: lineChartData,
		...
		this.constructor.prototype.load = function () {
			...
			this.chartType = options && options.chartType && options.chartType.get("value") ? options.chartType.get("value") : "line";
			...
			if(dataSource == "service"){
				this._proto._getLineChartDataFromService(this);
			}else{
		...
		this.constructor.prototype.change = function (event) {
			...
			if(event.property == "datasets" || event.property == "labels"){
				...
				this.myLine.config.data.datasets.forEach(function(dataset) {
				...
				var lineChartOptions = this._proto._getChartOptions(this);
				this.myLine.options = lineChartOptions;
				this.myLine.update();
